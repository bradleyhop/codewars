 ...
 *Example:
 *
 *highAndLow("1 2 3 4 5"); // return "5 1" highAndLow("1 2 -3 4 5"); // return "5
 *-3" highAndLow("1 9 3 4 -5"); // return "9 -5" Notes:
 *
...
function highAndLow(numbers){
    let testArr = numbers.split(" ").map( numb => {
        return parseInt(numb);
    });

    let highest = testArr[0], lowest = testArr[0];

    for (let i = 0; i < testArr.length; i++) {
        lowest  = Math.min(lowest, testArr[i]);
        highest = Math.max(highest, testArr[i]);
    }
