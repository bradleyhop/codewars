 ...
 * and return the values from the dictionary that start with the input string. If
 * there are more than 5 matches, restrict your output to the first 5 results. If
...
function autocomplete(input, dictionary) {
  let find = new RegExp(`^${input.replace(/[^a-z]/gi, '')}`, 'gi'),
      result = [];

  console.log(find.toString());

  for (let i = 0, m = 0; i < dictionary.length && m < 5; i++) {
    if (dictionary[i].match(find)){
...
console.log(autocomplete('a9/i', ['airplane','Airport','apple','ball']));
// ['airplane','airport']
